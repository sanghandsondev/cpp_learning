...
// upper_bound(): Trả về con trỏ / iterator trỏ đến phần tử đầu tiên có giá trị > X
// Array : auto p = upper_bound(a, a + n, X)            --> int value = *p;     int index = p - a;
// Vector: auto it = upper_bound(v.begin(), v.end(), X) --> int value = *it;    int index = it - v.begin();
...
int main(){
    ...
    cout << (it - v.begin()) << endl;           // it = 7 => v[7] = 9

    it = upper_bound(v.begin(), v.end(), 3);
    cout << (it - v.begin()) << endl;           // it = 5 => v[5] = 4
